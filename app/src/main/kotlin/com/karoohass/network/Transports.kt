...
sealed class TransportException(message: String) : Exception(message) {
    data object WifiUnavailable : TransportException("Wi-Fi connection is required")

    data class Failure(val detail: String) : TransportException(detail)
}
...
class DirectWifiTransport(private val context: Context) : HttpTransport {
            ...
            withTimeout(30_000) {
                kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                    ...
                    try {
                        ...
                        throw TransportException.Failure(exception.message ?: "Direct Wi-Fi request failed")
                    } finally {
    ...
    private companion object {
        const val MAX_DIRECT_RESPONSE_BYTES = 2_000_000
    }
}
                ...
                suspendCancellableCoroutine { continuation ->
                    ...
                    consumerId =
                        service.addConsumer<OnHttpResponse>(
                            ...
                            if (complete.body?.size ?: 0 > 100_000) {
                                continuation.resumeWith(Result.failure(TransportException.Failure("Response exceeds 100 KB")))
                            } else {
                                continuation.resume(HttpResponse(complete.statusCode, complete.headers, complete.body, complete.error))
                            }
                        }
