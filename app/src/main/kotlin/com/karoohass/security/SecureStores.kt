...
class TokenStore(context: Context) {
    ...
    fun load(): Tokens? = prefs.getString("tokens", null)?.let(::decrypt)?.split('\u0000')?.let { values ->
        values.takeIf { it.size == 3 }?.let { Tokens(it[0], it[1].ifBlank { null }, it[2].toLong()) }
    }
    fun clear() = prefs.edit().clear().apply()

    private fun key() = (KeyStore.getInstance("AndroidKeyStore").apply { load(null) }.getKey(keyAlias, null) ?: KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, "AndroidKeyStore").apply {
    ...
    private fun encrypt(value: String): String { val c = Cipher.getInstance(cipher); c.init(Cipher.ENCRYPT_MODE, key()); return Base64.encodeToString(c.iv + c.doFinal(value.toByteArray()), Base64.NO_WRAP) }
    private fun decrypt(value: String): String { val input = Base64.decode(value, Base64.NO_WRAP); val c = Cipher.getInstance(cipher); c.init(Cipher.DECRYPT_MODE, key(), GCMParameterSpec(128, input.copyOfRange(0, 12))); return String(c.doFinal(input.copyOfRange(12, input.size))) }
}
...
class PinStore(context: Context) {
    ...
    fun set(pin: String) { require(pin.length in 4..6 && pin.all(Char::isDigit)); val salt = ByteArray(16).also(random::nextBytes); prefs.edit().putString("salt", b64(salt)).putString("hash", b64(derive(pin, salt))).putInt("failures", 0).putLong("lockedUntil", 0).putInt("lockouts", 0).apply() }
    fun verify(pin: String, now: Long = System.currentTimeMillis()): PinResult {
        ...
        if (MessageDigest.isEqual(derive(pin, salt), expectedHash)) { prefs.edit().putInt("failures", 0).putInt("lockouts", 0).putLong("lockedUntil", 0).apply(); return PinResult.Success }
        val failures = prefs.getInt("failures", 0) + 1
        if (failures < 5) { prefs.edit().putInt("failures", failures).apply(); return PinResult.Failed }
...
sealed interface PinResult { data object Success : PinResult; data object Failed : PinResult; data class Locked(val remainingMs: Long) : PinResult }
